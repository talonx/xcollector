...
def main():
    ...
    while signal_received is None:
        ...
        if (((fields[0] == "CPU") or (re.match("[0-9][0-9]:[0-9][0-9]:[0-9][0-9]", fields[0]))) and (
                (collect_every_cpu and re.match("[0-9]+:?", fields[1])) or (
                (not collect_every_cpu) and re.match("all:?", fields[1])))):
            if ((fields[1] == "all") or (fields[1] == "0")):
                timestamp = int(time.time())
            cpuid = fields[1].replace(b":", b"")
            cpuuser = fields[2]
            cpunice = fields[3]
            cpusystem = fields[4]
