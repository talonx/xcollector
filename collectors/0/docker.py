...
def getnameandimage(containerid):
    ...
    try:
        ...
        if (r != 0):
            utils.err("Can not connect to %s" % (DOCKER_SOCK))
        else:
            ...
            try:
                ...
                try:
                    ...
                    utils.err(containerid + " has no Image field")
            except:
                utils.err("Can not load json")

    except socket.timeout as e:
        utils.err("Socket: %s" % e)
...
def readdockerstats(path, containerid):
    ...
    for file_stat in os.listdir(path):
        if (os.path.isfile(path + "/" + file_stat) \
                and ((file_stat in list(proc_names.keys())) or (file_stat in list(proc_names_to_agg.keys())))):
            try:
            ...
            # proc_name
            if (file_stat in list(proc_names.keys())):
                datatosend = None
                f_stat.seek(0)
                for line in f_stat:
                    tags = None
            ...
            # proc_names_to_agg
                ...
                if (file_stat in list(proc_names_to_agg.keys())):
