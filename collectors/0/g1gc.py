...
pattern_map = {
    GC_START_TIME_PATTERN: re.compile(
        '^(20\d\d)\-([01]\d)\-([0123]\d)T([012]\d):([012345]\d):([012345]\d).\d\d\d([+-]\d\d\d\d):\s*%s:\s*\[\s*(.+)' %
    ...
    SCAN_RS_PATTERN: re.compile('\s*\[Scan RS \(ms\): Min: (%s), Avg: (%s), Max: (%s)' % (
    FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN)),
    OBJECT_COPY_PATTERN: re.compile('\s*\[Object Copy \(ms\): Min: (%s), Avg: (%s), Max: (%s)' % (
    FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN)),
    # [Eden: 3584.0M(3584.0M)->0.0B(3584.0M) Survivors: 512.0M->512.0M Heap: 91.2G(100.8G)->87.9G(100.8G)]
    ALLOCATION_PATTERN: re.compile(
        '^\s*\[Eden: (%s)([BMG])\(\d+(?:\.\d+)?[MG]\)\->(%s)([BMG])\((%s)([BMG])\) Survivors: (%s)([BMG])\->(%s)([BMG]).+Heap: (%s)G\((%s)G\)\->(%s)G\((%s)G\).+' % (
        FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN,
        FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN, FLOAT_NUMBER_PATTERN)),
    # [Free CSet: 1.1 ms]
...
def concurrent_root_region_scan_handler(prefix, log_line, timestamp, collector, file_handler):
    ...
    collect_metric_with_prefix(prefix, "gc.g1.concurrent_root_region_scan %s %s", timestamp,
                               concurrent_root_region_scan_time, collector)
...
def allocation_handler(prefix, matcher, timestamp, collector, file_handler):
    ...
    collect_metric_with_prefix(prefix, "gc.g1.promotion %s %s", timestamp,
                               (eden_before - eden_after) - (heap_before - heap_after), collector)
    collect_metric_with_prefix(prefix, "gc.g1.heap_ratio.before %s %s", timestamp, heap_before / heap_total_size_before,
                               collector)
    collect_metric_with_prefix(prefix, "gc.g1.heap_ratio.after %s %s", timestamp, heap_after / heap_total_size_after,
                               collector)
...
def initial_mark_handler(prefix, log_line, timestamp, collector, file_handler):
    ...
    collect_metric_with_prefix(prefix, "gc.g1.duration %s %s phase=initial-mark", timestamp, initial_mark_pause_time,
                               collector)
