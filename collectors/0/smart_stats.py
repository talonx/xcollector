...
def alarm_handler(signum, frame):
    utils.err("Program took too long to run, consider increasing its timeout.")
    raise Alarm()
...
def is_adaptec_driver_broken():
    ...
    if arcconf.returncode != 0:
        ...
        if arcconf.returncode == 127:
            ...
            return False
        utils.err("arcconf unexpected error %s" % arcconf.returncode)
        return True
    for line in arcconf_output.split("\n"):
        ...
        if fields[0] == "Driver" and fields[2] in BROKEN_DRIVER_VERSIONS:
            utils.err("arcconf indicates broken driver version %s" % fields[2])
            return True
    return False
...
def is_3ware_driver_broken(drives):
    ...
    for i in reversed(range(len(drives))):
        ...
        if "supports SMART and is Disabled" in smart_output:
            utils.err("SMART is disabled for %s" % drive)
            del drives[i]  # We're iterating from the end of the list so this is OK.
        signal.alarm(0)
    if not drives:
        utils.err("None of the drives support SMART. Are they SAS drives?")
        return True
    return False
        ...
        for drive in drives:
            ...
            if smart_ctl.returncode != 0:
                if smart_ctl.returncode == 127:
                    ...
                    utils.err("Command exited with: %d" % smart_ctl.returncode)
